...
"""
	...
	var valid: bool = parsed.get("ok", false) and parsed.coordinates_mm.size() == 3 and parsed.pickup_dock.size() == 12 and is_equal_approx(parsed.pickup_dock[0], 399.761) and is_equal_approx(parsed.pitch_degrees[1], 10.0) and is_equal_approx(parsed.joint_degrees[0][1], -104.7) and not parsed["overlays"]["status"] and not parsed["overlays"]["translation"] and parsed.waypoint_events[0].size() == 1 and parsed.waypoint_events[2][0]["key"] == "pickup_thin_side" and parsed.waypoint_events[2][1]["key"] == "return_product" and planned
